...
public abstract class OllamaConsole(IOllamaApiClient ollama)
{
    ...
    public static string AiTextColor { get; } = "grey70";
    
    public static string DebugTextColor { get; } = "darkorange";

    public static string START_NEW_COMMAND { get; } = "/new";

    public static string EXIT_COMMAND { get; } = "/exit";
    
    public static string DEBUG_COMMAND { get; } = "/debug";
	...
	protected void WriteChatHeader()
	{            
		AnsiConsole.Write(new Rule("laxchat").LeftJustified());
		#if DEBUG
		AnsiConsole.MarkupLine($"[{DebugTextColor}]DEBUG[/]");
		#endif
